...
console.log('🧪 Testing Calendar API Real Endpoint...\n');

async function testCalendarAPIReal() {
  try {
    ...
    // Step 4: Test with Free plan
    console.log('\n4️⃣ Testing with FREE plan:');
    ...
    // Step 5: Test subscription controller directly
    console.log('\n5️⃣ Testing subscription controller directly:');
    
    // Create a proper Express-like response object
    ...
    try {
      ...
      console.log(`   API Response: ${JSON.stringify(mockRes2, null, 2)}`);
    } catch (error) {
    ...
    const standardWorking = effectivePlan.canAccessCalendar === true && 
                          middlewareResult?.allowed === true;
    
    const freeWorking = freeEffectivePlan.canAccessCalendar === false && 
                       freeMiddlewareResult.status === 402;
    
    if (standardWorking && freeWorking) {
      console.log('✅ BACKEND IS WORKING PERFECTLY');
      console.log('   - Standard plan: Calendar access ALLOWED');
      console.log('   - Free plan: Calendar access BLOCKED');
      console.log('   - Middleware working correctly');
      console.log('   - Plan inheritance working correctly');
      ...
      console.log('   6. Check if client is calling /api/subscriptions/check-feature-access');
      
    } else {
      console.log('❌ BACKEND HAS ISSUES');
      console.log('   - Standard plan: NOT working correctly');
      console.log('   - Free plan: NOT working correctly');
      console.log('   - Middleware or plan inheritance has issues');
    }
    
    console.log('\n📝 NEXT STEPS:');
    console.log('1. Test calendar access in browser with Aniket2');
    console.log('2. Check browser console for errors');
    console.log('3. Check network tab for /api/subscriptions/check-feature-access');
    console.log('4. Look for free plan popup behavior');
    console.log('5. Verify calendar component is checking canAccessCalendar');
