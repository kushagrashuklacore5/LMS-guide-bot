...
db.all('SELECT * FROM vendors ORDER BY id', [], (err, vendors) => {
  ...
  const testVendors = vendors.filter(v => vendor.university_id === 1 || vendor.university_id === null);
  
  if (testVendors.length > 0) {
    console.log(`\nFound ${testVendors.length} potential test vendors with university_id = 1:`);
    testVendors.forEach(vendor => {
      console.log(`- ID: ${vendor.id}, Name: ${vendor.name}`);
    });

    console.log('\nTo remove test vendors, run:');
    console.log('DELETE FROM vendors WHERE university_id = 1;');
    console.log('Or use the cleanup function below...');
    
    // Optional: Clean up test vendors
    console.log('\nCleaning up test vendors...');
    db.run('DELETE FROM vendors WHERE university_id = 1', function(err) {
      if (err) {
