...
async function testMentorCalendarResultsFixed() {
  console.log('🧪 Testing Mentor Calendar & Results Access (Fixed)...\n');
  ...
  try {
    ...
    if (rishiLogin.status === 200) {
      ...
      try {
        ...
        console.log('✅ Mentor Courses:', coursesData.length, 'courses found');
      ...
      try {
        ...
        classroomsData = classroomsResponse.data.data || classroomsResponse.data;
        console.log('✅ Mentor Classrooms:', classroomsData.length, 'classrooms found');
        classroomsData.forEach((classroom, index) => {
          console.log(`   ${index + 1}. ${classroom.name} (ID: ${classroom.id})`);
        });
        
        if (classroomsData.length > 0) {
          const testClassroom = classroomsData[0];
      ...
      // Test 8: Check if we can get all students (not just classroom-specific)
      console.log('\n👥 Testing All Students Access...');
      try {
        const allStudentsResponse = await axios.get('http://127.0.0.1:5002/api/users?role=student', {
          headers: { 
            'Authorization': `Bearer ${rishiToken}`
          }
        });
        const allStudents = allStudentsResponse.data.data || allStudentsResponse.data;
        console.log('✅ All Students:', allStudents.length, 'students found');
        allStudents.slice(0, 5).forEach((student, index) => {
          console.log(`   ${index + 1}. ${student.name} (${student.email})`);
        });
      } catch (error) {
        console.log('❌ All Students Access:', error.response?.status, error.response?.data?.message);
      }
...
testMentorCalendarResultsFixed();
