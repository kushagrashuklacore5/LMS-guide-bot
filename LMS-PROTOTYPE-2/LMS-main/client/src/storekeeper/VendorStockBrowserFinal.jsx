...
import { 
  ...
  DollarSign, Grid, List, Download, RefreshCw, XCircle, Trash2, Send
} from 'lucide-react';

const VendorStockBrowserFinal = () => {
  ...
  const addToRequest = (item) => {
    setRequestItems(prev => {
      const existingItem = prev.find(reqItem => reqItem.id === item.id);
      if (existingItem) {
        return prev.map(reqItem => 
          reqItem.id === item.id 
            ? { ...reqItem, requestQuantity: reqItem.requestQuantity + 1 }
            : reqItem
        );
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Browse vendor inventory</p>
        </div>
        <div className="flex items-center space-x-3">
          {requestItems.length > 0 && (
            ...
            >
              ...
              <span>Request ({requestItems.length})</span>
            </button>
          )}
          <div className="flex items-center space-x-2">
            <button
              onClick={() => setViewMode('grid')}
              className={viewMode === 'grid' ? 'p-2 rounded bg-blue-100 text-blue-600' : 'p-2 rounded text-gray-600'}
            >
              <Grid size={20} />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={viewMode === 'list' ? 'p-2 rounded bg-blue-100 text-blue-600' : 'p-2 rounded text-gray-600'}
            >
              <List size={20} />
            </button>
          </div>
      ...
      {showSuccess && (
        <div className="fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg flex items-center space-x-2 z-50">
          <CheckCircle size={20} />
          <span>Item added to request!</span>
        </div>
      )}
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-6">
                {stockItems.map((item) => {
                  ...
                  const statusClass = stockStatus.status === 'out' ? 'text-red-600' :
                                       stockStatus.status === 'low' ? 'text-yellow-600' :
                                       'text-green-600';
                  const bgClass = stockStatus.status === 'out' ? 'bg-red-50' :
                        ...
                        <tr key={item.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center space-x-2">
                              ...
                              <span className={'text-xs px-2 py-1 rounded-full font-medium ' + bgClass}>
                                {stockStatus.text}
                              </span>
                            </div>
                          </td>
...
const RequestModal = ({ items, vendor, onClose, onUpdateQuantity, onRemove, onSuccess }) => {
  ...
  const [formData, setFormData] = useState({
    title: `Request from ${vendor?.name || 'Vendor'}`,
    description: '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.success) {
          alert('Request submitted successfully! Vendors have been notified and can view this request in their portal.');
          onSuccess();
        }
      }
    } catch (error) {
...
export default VendorStockBrowserFinal;
