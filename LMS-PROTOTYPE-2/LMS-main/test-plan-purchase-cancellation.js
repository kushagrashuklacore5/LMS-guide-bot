...
async function testPlanPurchaseCancellation() {
  try {
    ...
    // Insert new subscription (simulate purchase)
    const purchaseResult = await new Promise((resolve, reject) => {
      ...
      `, [
        ...
        purchaseData.startDate, // createdAt
        purchaseData.startDate  // updatedAt
      ], function(err) {
        if (err) reject(err);
    ...
    console.log(`   ✅ Purchase recorded: ID ${purchaseResult.id}`);
    ...
    // Step 8: Check subscription history
    console.log('\n8️⃣ Checking subscription history:');
    
    const allSubscriptions = await new Promise((resolve, reject) => {
      db.all('SELECT * FROM subscriptions WHERE superadminId = 33 ORDER BY createdAt DESC', (err, rows) => {
        if (err) reject(err);
        else resolve(rows);
      });
    });
    ...
    allSubscriptions.forEach((sub, index) => {
      console.log(`   ${index + 1}. ${sub.planName} (${sub.planType}) - ${sub.status} - ${sub.updatedAt}`);
    });
    ...
    console.log(`✅ Total subscription records: ${allSubscriptions.length}`);
    ...
    if (purchaseStored && usersUpdated && cancellationStored && usersUpdatedAfterCancellation) {
      ...
      console.log('   ✅ Every purchase creates a new record');
      console.log('   ✅ Every cancellation updates the record');
    ...
    console.log('   - Purchase: ✅ Creates/updates subscription record');
    console.log('   - Cancellation: ✅ Updates existing subscription record');
    console.log('   - User plans: ✅ Automatically synchronized');
    console.log('   - History: ✅ Maintains complete subscription history');
